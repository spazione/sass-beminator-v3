...
test('source architecture has one module variable, centralized writes, and no native extension primitives', () => {
  ...
  assert.deepEqual([...source.matchAll(/^\$([\w-]+):/gm)].map((match) => match[1]), ['-context-stack']);
